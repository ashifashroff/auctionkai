...
function renderStatement(array $m, array $s, array $auction, string $payStatus = 'unpaid', array $brand = [], bool $useLandscape = false): string {
    ...
    <div class='page'{$landscapeStyle}{$landscapePage}>
      ...
      <div class='hdr'>
        <div><div class='brand'>" . h($brand['brand_name']) . " <span>Settlement Statement</span></div><div class='sub'>Settlement Statement · " . h($auction['name']) . $exp . "</div>" . ((!empty($brand['brand_email']) || !empty($brand['brand_phone'])) ? "<div style='font-size:11px;color:#666;margin-top:4px'>" . (!empty($brand['brand_email']) ? '✉ ' . h($brand['brand_email']) . ' ' : '') . (!empty($brand['brand_phone']) ? '📞 ' . h($brand['brand_phone']) : '') . "</div>" : "") . "</div>
