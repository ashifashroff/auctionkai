...
**Vehicles** — add, edit, and delete without page reload (everything's AJAX). Toggle sold/unsold with one click — green/red flash animation, no page reload. Live payout preview below the add vehicle form shows estimated net payout in real-time as you type. Paginated vehicles table (10/25/50/100 per page). Real-time search filter by lot, make, model, member name. AJAX pagination — no full page reload. Skeleton loading animation while fetching. Stays on same page after add/edit/delete. Nagare fee only appears for unsold vehicles — sold vehicles get sold price, tax, recycle, listing fee, and sold fee instead. Duplicate lot numbers are caught in real-time before submission. Inline confirmation for vehicle delete — no browser popups.
...
**Statements** — members with sold vehicles appear as gold statement cards. Nagare-only members (all vehicles unsold) appear with a distinct red-tinted "✗ No Sales" card showing nagare fees, commission toggle, and amount owed. Optional commission checkbox for nagare-only members — unchecked by default, persisted to DB, reflected in PDF. Full breakdown from gross sales down to net payout. Special fees included in deductions/additions. Download individual or all-statement PDFs. Email button sends via AJAX. WhatsApp button — one click opens WhatsApp with full settlement statement pre-filled. Shareable statement links — generate a unique URL per member per auction, PIN protected, valid for 14 days. Payment status tracking (Unpaid / Partial / Paid) with one-click update. PAID/PARTIAL stamp on PDF statements.
...
**🔒 Forgot Password** — request a password reset link by email. Reset with a new password (minimum 8 characters). Password strength indicator shows Weak/Fair/Good/Strong in real-time. Eye toggle on all password fields to show/hide.
